...
class PortfolioManager:
    ...
    def open_position(
        ...
        # Enforce the global slot invariant before mutating any position state.
        open_count = len(self._get_open_positions())
        if open_count >= self.max_positions:
            self.log_info(
                f"[RISK] Position limit reached ({open_count}/{self.max_positions}). "
                f"Refus d'ouverture pour {asset}."
            )
            try:
                if hasattr(self, "metrics") and self.metrics:
