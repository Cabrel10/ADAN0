...
class PortfolioManager:
    ...
    def close_position(self, asset, price):
        ...
        # Patch Gugu & March - Mise à jour série victoires
        try:
            if realized_pnl > 0:
                self.update_winning_streak(trade_won=True)
            elif realized_pnl < 0:
                self.update_winning_streak(trade_won=False, trade_lost=True)
        except Exception as e:
            logger.debug(f"[MARCH] Error updating winning streak: {e}")
    ...
    # Patch Gugu & March - Tracking des séries de victoires
    def update_winning_streak(self, trade_won: bool, trade_lost: bool = False):
        """Met à jour le compteur de série de victoires"""
        if not hasattr(self, '_winning_streak_count'):
            self._winning_streak_count = 0

        if trade_won:
            self._winning_streak_count += 1
            logger.debug(f"[MARCH] Winning streak extended to {self._winning_streak_count}")
        elif trade_lost:
            if self._winning_streak_count >= 3:
                logger.info(f"[MARCH] End of winning streak: {self._winning_streak_count} trades")
            self._winning_streak_count = 0

    @property
    def winning_streak_count(self) -> int:
        """Retourne le nombre actuel de victoires consécutives"""
        return getattr(self, '_winning_streak_count', 0)
