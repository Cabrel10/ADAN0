...
def resolve_agent_close_gate(
    *,
    ...
    """Resolve structural close gates while preserving the policy's exit authority.

    Returns ``(blocked, reason)``. In V20, ``exit_authority=True`` makes the
    decision budget and break-even barrier advisory after HOLD_MIN. This is a
    risk-reduction path: a policy-requested loss cut must not be converted to
    HOLD merely because the position is currently unprofitable.
    """
    if exit_authority:
