...
class DynamicBehaviorEngine:
    ...
    def __init__(
        ...
        self.smart_logger = create_smart_logger(
            self.worker_id, total_workers=4, logger_name="dynamic_behavior_engine"
        )
    ...
    def _merge_configs(
        self, result: Dict[str, Any], update: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
    ...
    def update_risk_parameters(
        self, market_data: Dict[str, Any], portfolio_value: float
    ) -> Dict[str, float]:
        """
        ...
        try:
            ...
            if (
                confidence
                > self.config.get("market_regime_detection", {}).get(
                    "regime_confidence_threshold", 0.7
                )
                ...
                logger.info(
                    f"Changement de régime détecté: {new_regime.upper()} (Confiance: {confidence * 100:.1f}%)"
                )
            ...
            if tier:
                ...
                position_size_pct = (
                    min_exposure_pct
                    + (max_exposure_pct - min_exposure_pct) * normalized_risk
                )
                ...
                min_trade_value = self.config.get("risk_management", {}).get(
                    "min_trade_value", 11.0
                )
                if (
                    portfolio_value > 0
                    and (position_size_pct * portfolio_value) < min_trade_value
                ):
                    ...
                    if (min_trade_value / portfolio_value) <= max_tier_size_pct:
                        ...
                        logger.warning(
                            f"Conflit de configuration pour le palier {tier.get('name')}: min_trade_value "
                ...
                position_size_multiplier = regime_params.get(
                    "position_size_multiplier", 1.0
                )
                risk_level = self.state.get("current_risk_level", 1.0)
                position_size_pct = np.clip(
                    risk_params.get("initial_position_size", 0.1)
            ...
            vol_adjustment = np.clip(
                (volatility - min_vol) / (max_vol - min_vol + 1e-6) * 1.5 + 0.5,
                0.5,
                2.0,
            )
            ...
            stop_loss_pct = np.clip(
                base_sl_pct * sl_multiplier * vol_adjustment, min_sl_pct, max_sl_pct
            )
            ...
            if (
                hasattr(self, "env")
                and hasattr(self.env, "positions_count")
                and self.frequency_config
            ):
                frequency_adjustment = self._calculate_frequency_adjustment(
                    self.env.positions_count
                )
            ...
            logger.error(
                f"Erreur lors de la mise à jour des paramètres de risque: {e}",
                exc_info=True,
            )
    ...
    def _calculate_frequency_adjustment(
        self, positions_count: Dict[str, int]
    ) -> Dict[str, float]:
        """
    ...
    def compute_dynamic_modulation(
        self, env=None, risk_horizon: float = 0.0
    ) -> Dict[str, Any]:
        """
        ...
        if not hasattr(self, "state") or self.state is None:
            logger.error(
                f"[DBE Worker {self.worker_id}] CRITICAL: State lost in compute_dynamic_modulation! Forcing emergency initialization"
            )
            self.state = {
                "current_step": 0,
                "market_regime": "NEUTRAL",
            ...
            logger.debug(
                f"[DBE Worker {self.worker_id}] State preserved, step: {self.state.get('current_step', 0)}"
            )

        # Log decision (filtrage fait dans log_info)
        self.log_info(
            f"[DBE_DECISION] Computing dynamic modulation - Regime: {self.state.get('market_regime', 'unknown')}, Volatility: {self.state.get('volatility', 0.0)}, Step: {self.state.get('current_step', 0)}"
        )

        try:
            # Initialisation des paramètres de base
            mod = {
                "sl_pct": self.config.get("risk_parameters", {}).get(
                    "base_sl_pct", 0.02
                ),
                "tp_pct": self.config.get("risk_parameters", {}).get(
                    "base_tp_pct", 0.04
                ),
                "reward_boost": 1.0,
                ...
                "leverage": self.config.get("position_sizing", {}).get(
                    "base_leverage", 1.0
                ),
            ...
            self._compute_risk_parameters(self.state, mod, risk_horizon)
    ...
    def _adjust_position_size_aggressively(self, mod: Dict[str, Any], env) -> None:
        ...
        for tf in ["5m", "1h", "4h"]:
            ...
            if (
                ...
                logger.info(
                    f"[DBE_POSITION Worker {getattr(self, 'worker_id', 0)}] Increasing position size by 30% for {tf} - insufficient trades"
                )
            elif (
    ...
    def _adjust_learning_parameters(self, mod: Dict[str, Any]) -> None:
        ...
        )  # Limite le facteur de risque entre 0.1 et 10.0
        ...
        try:
            ...
            mod["learning_rate"] = max(
                1e-8, min(base_lr, lr_range[1] * 2.0)
            )  # Ne dépasse pas le double du max
    ...
    def _detect_market_regime(
        self, rsi: float, adx: float, ema_ratio: float, atr: float, atr_pct: float
    ) -> str:
        """
    ...
    def _calculate_sharpe_ratio(
        self, returns_tuple: Tuple[float, ...], risk_free_rate: float = 0.0
    ) -> float:
    ...
    @classmethod
    def load_state(
        cls, filepath: Union[str, Path], finance_manager: Optional[Any] = None
    ...
    def calculate_trade_parameters(
        ...
        try:
            ...
            # Ajustement en fonction de l'horizon de risque de l'agent
            # Pour le SL: horizon court (-1) = SL plus serré, horizon long (1) = SL plus large
            sl_multiplier_rh = 1.0 - (
                risk_horizon * 0.3
            )  # Ex: -1 -> 1.3, 0 -> 1.0, 1 -> 0.7
            # Pour le TP: horizon court (-1) = TP plus serré, horizon long (1) = TP plus large
            tp_multiplier_rh = 1.0 + (
                risk_horizon * 0.3
            )  # Ex: -1 -> 0.7, 0 -> 1.0, 1 -> 1.3

            # Appliquer les multiplicateurs
            sl_pct *= sl_multiplier_rh
            tp_pct *= tp_multiplier_rh

            # Journaliser l'ajustement
            logger.debug(
                f"[DBE_CALC] Paramètres de risque ajustés pour horizon={risk_horizon:.2f}: SL={sl_pct:.2%} (x{sl_multiplier_rh:.2f}), TP={tp_pct:.2%} (x{tp_multiplier_rh:.2f})"
            )
    ...
    def _compute_risk_parameters(
        self, state: Dict[str, Any], mod: Dict[str, Any], risk_horizon: float = 0.0
    ) -> None:
        """
        ...
        try:
            ...
            winrate_factor = min(
                1.0, max(0.0, (win_rate - 0.4) / 0.6)
            )  # 0% à 100% pour winrate de 0.4 à 1.0

            # 2. Facteur de drawdown (pénalise les périodes de pertes)
            drawdown_factor = max(
                0.0, 1.0 - (current_drawdown / 0.1)
            )  # 100% à 0% pour drawdown de 0% à 10%

            # 3. Facteur de volatilité (pénalise la volatilité élevée)
            volatility_factor = 1.0 - min(
                1.0, volatility * 5
            )  # 100% à 0% pour volatilité de 0% à 20%
