...
# =====================================================================
# SOTA 2026: WorldModelPPO -- PPO with REAL auxiliary forward-prediction loss
# The auxiliary loss IS backpropagated through the feature extractor,
# forcing the CNN backbone to learn predictive representations.
# =====================================================================
try:
    ...
    class WorldModelPPO(_BasePPO):
        ...
        After each PPO train() epoch, an extra gradient step is performed:
          ...
          2. Run it through the policy to populate _last_aux_prediction.
          3. Compute target = actual next-step log-return from rewards/values.
          4. aux_loss = MSE(prediction, target) * aux_loss_coef.
          5. Backpropagate and step the optimiser.

        This ensures the CNN/Cross-Attention backbone learns to predict
        future returns, not just maximise immediate reward.
        """
