...
class ChunkedDataLoader:
    ...
    def __init__(self, config, worker_config, worker_id=0):
        ...
        for tf in self.timeframes:
            if isinstance(cfg_chunk_sizes, dict) and tf in cfg_chunk_sizes:
                ...
                self.chunk_sizes[tf] = int(default_chunk_sizes.get(tf, 25_000))
    ...
    def load_chunk(self, chunk_idx: int = 0) -> Dict[str, Dict[str, pd.DataFrame]]:
        ...
        try:
            ...
            # Reindex 1h/4h onto the 5m index (ffill) so that at every
            # row i ALL timeframes refer to the same wall-clock minute.
            # This prevents the "time-travel bug" where misaligned
            # indices let the agent peek into the future.
            data = self._align_master_clock(data)

            # Validation du chunk avec le nouveau système
            if data:
                # Extract actual timestamps from loaded data
                all_timestamps = []
                for asset_data in data.values():
                    for tf_data in asset_data.values():
                        if hasattr(tf_data, "index") and len(tf_data.index) > 0:
                            all_timestamps.extend(
                                [tf_data.index.min(), tf_data.index.max()]
                            )
    ...
    def _align_master_clock(
        self, data: Dict[str, Dict[str, pd.DataFrame]]
    ) -> Dict[str, Dict[str, pd.DataFrame]]:
        """Align all timeframes to the 5m Master Clock using forward-fill.

        For each asset that has a '5m' DataFrame, reindex '1h' and '4h'
        onto the 5m DatetimeIndex via ``ffill()``.  If the indices already
        share the same timestamps (e.g. the dataset was generated by
        ``generate_colab_dataset.py``), this is a harmless no-op.

        Returns the (potentially mutated) data dict.
        """
        MASTER_TF = "5m"

        for asset, tf_dict in data.items():
            if MASTER_TF not in tf_dict:
                continue  # Nothing to align

            master_df = tf_dict[MASTER_TF]
            if master_df.empty:
                continue

            master_idx = master_df.index

            # Ensure master index is tz-naive for comparison
            if hasattr(master_idx, "tz") and master_idx.tz is not None:
                master_idx_naive = master_idx.tz_localize(None)
            else:
                master_idx_naive = master_idx

            for tf_name, tf_df in tf_dict.items():
                if tf_name == MASTER_TF or tf_df.empty:
                    continue

                # Make the higher-TF index tz-naive too
                tf_idx = tf_df.index
                if hasattr(tf_idx, "tz") and tf_idx.tz is not None:
                    tf_df = tf_df.copy()
                ...
                # Reindex onto master clock with forward-fill
                before_len = len(tf_df)
                tf_aligned = tf_df.reindex(master_idx_naive, method="ffill")
                # Drop leading rows that couldn't be filled
                tf_aligned = tf_aligned.dropna(subset=[
                    c for c in tf_aligned.columns
                    if c.lower() == "close"
                ] or tf_aligned.columns[:1])

                # Keep only the intersection with master
                common_idx = master_idx_naive.intersection(tf_aligned.index)
                tf_aligned = tf_aligned.loc[common_idx]

                logger.info(
                    f"[MASTER_CLOCK] {asset}/{tf_name}: "
                    f"reindexed {before_len} -> {len(tf_aligned)} rows "
                    f"(aligned to {MASTER_TF})"
                )
                tf_dict[tf_name] = tf_aligned

            # Trim master to the common range (all TFs must have data)
            common_range = master_idx_naive
            for tf_name, tf_df in tf_dict.items():
                if tf_df.empty:
                    continue
                tf_idx = tf_df.index
                if hasattr(tf_idx, "tz") and tf_idx.tz is not None:
                    tf_idx = tf_idx.tz_localize(None)
                common_range = common_range.intersection(tf_idx)

            if len(common_range) < len(master_idx_naive):
                for tf_name in list(tf_dict.keys()):
                    tf_df = tf_dict[tf_name]
                    if tf_df.empty:
                        continue
                    idx = tf_df.index
                    if hasattr(idx, "tz") and idx.tz is not None:
                        idx = idx.tz_localize(None)
                    mask = idx.isin(common_range)
                    tf_dict[tf_name] = tf_df.loc[mask]

                logger.info(
                    f"[MASTER_CLOCK] {asset}: trimmed to {len(common_range)} "
                    f"common timestamps across all timeframes"
                )
