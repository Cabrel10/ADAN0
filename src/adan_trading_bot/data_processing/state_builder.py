...
class StateBuilder:
    ...
    #   [1]  trend            -- EMA crossover direction (-1..1)
    #   [2]  adx              -- ADX / 100  (0-1)
    #   [3]  hmm_bull_prob    -- HMM state 0 posterior probability (0-1)
    #   [4]  hmm_sideways_prob-- HMM state 1 posterior probability (0-1)
    #   [5]  hmm_bear_prob    -- HMM state 2 posterior probability (0-1)
    #   [6]  drawdown         -- current portfolio drawdown (0-1)
    #   [7]  candle_progress  -- progress within episode (0-1)
    #   [8]  sin_hour         -- sin(2*pi*hour/24)
    #   [9]  cos_hour         -- cos(2*pi*hour/24)
    #   [10] sin_dow          -- sin(2*pi*dayofweek/7)
    ...
    def build_context_vector(
        ...
        Args:
            ...
            hmm_probs: Optional (3,) array of HMM state posteriors [bull, sideways, bear].
        ...
        if data is None:
            # Uniform HMM prior even when data is missing
            context[3] = 0.33
            context[4] = 0.33
            context[5] = 0.34
        ...
        try:
            for tf in self.timeframes:
                ...
                # [0] Volatility from ATR
                for atr_col in ["ATR_14", "atr_14", "atr_20"]:
                ...
                # [1] Trend from EMA crossover (try multiple column names)
                for ema_short, ema_long in [("EMA_12", "EMA_26"), ("ema_20_ratio", None)]:
                    if ema_long and ema_short in df.columns and ema_long in df.columns:
                        e12 = df[ema_short].iloc[idx]
                        e26 = df[ema_long].iloc[idx]
                        if np.isfinite(e12) and np.isfinite(e26) and e26 != 0:
                            diff = (e12 - e26) / abs(e26)
                            context[1] = float(np.clip(diff * 10, -1.0, 1.0))
                        break
                    elif ema_long is None and ema_short in df.columns:
                        val = df[ema_short].iloc[idx]
                        if np.isfinite(val):
                            context[1] = float(np.clip(val, -1.0, 1.0))
                        break

                # [2] ADX
                for adx_col in ["ADX_14", "adx_14"]:
            ...
            # [3,4,5] HMM posterior probabilities (bull, sideways, bear)
            if hmm_probs is not None and len(hmm_probs) >= 3:
                for i in range(3):
                    context[3 + i] = float(np.clip(hmm_probs[i], 0.0, 1.0))
            else:
                # Uniform prior
                context[3] = 0.33
                context[4] = 0.33
            ...
            # [6] Drawdown from portfolio manager
            if portfolio_manager is not None:
                try:
                    if hasattr(portfolio_manager, "get_metrics"):
                        ...
                        context[6] = float(np.clip(abs(dd) / 100.0, 0.0, 1.0))
                    elif hasattr(portfolio_manager, "get_state_vector"):
                        state = portfolio_manager.get_state_vector()
                        if len(state) > 4:
                            context[6] = float(np.clip(abs(state[4]) / 100.0, 0.0, 1.0))
                except Exception:
                    pass

            # [7] Candle progress: position within data chunk (0..1)
            try:
                for tf in self.timeframes:
                    if tf in data and not data[tf].empty:
                        total_len = len(data[tf])
                        if total_len > 1:
                            context[7] = float(np.clip(
        ...
        try:
            ...
            if timestamp is not None:
                ...
                context[8]  = float(np.sin(2.0 * np.pi * hour / 24.0))
                context[9]  = float(np.cos(2.0 * np.pi * hour / 24.0))
                context[10] = float(np.sin(2.0 * np.pi * dow / 7.0))
                context[11] = float(np.cos(2.0 * np.pi * dow / 7.0))
                context[12] = float(np.sin(2.0 * np.pi * dom / 31.0))
                context[13] = float(np.cos(2.0 * np.pi * dom / 31.0))
        except Exception as e:
