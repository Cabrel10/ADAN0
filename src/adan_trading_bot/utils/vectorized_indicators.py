...
def vectorized_bollinger_bands(
    prices: np.ndarray, window: int = 20, std_dev: float = 2.0
) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
...
def vectorized_stochastic(
    ...
    for i in range(k_window - 1, n):
        ...
        if highest_high == lowest_low:
            ...
            k_percent[i] = (
                (close[i] - lowest_low) / (highest_high - lowest_low)
            ) * 100.0
...
def vectorized_supertrend(
    ...
    for i in range(atr_window, n):
        ...
        if close[i] > upper_band[i]:
            ...
            trend[i] = (
                trend[i - 1] if trend[i - 1] == lower_band[i - 1] else upper_band[i]
            )
...
@njit(cache=True)
def vectorized_ichimoku(
    high: np.ndarray,
    low: np.ndarray,
    close: np.ndarray,
    conversion_window: int = 9,
    base_window: int = 26,
    leading_span_b_window: int = 52,
    ...
    for i in range(conversion_window - 1, n):
        tenkan_sen[i] = (
            np.max(high[i - (conversion_window - 1) : i + 1])
            + np.min(low[i - (conversion_window - 1) : i + 1])
        ) / 2

    for i in range(base_window - 1, n):
        kijun_sen[i] = (
            np.max(high[i - (base_window - 1) : i + 1])
            + np.min(low[i - (base_window - 1) : i + 1])
        ) / 2
    ...
    for i in range(leading_span_b_window - 1, n):
        ...
        senkou_span_b[i] = (
            np.max(high[i - (leading_span_b_window - 1) : i + 1])
            + np.min(low[i - (leading_span_b_window - 1) : i + 1])
        ) / 2
...
@njit(cache=True)
def vectorized_rolling_correlation(
...
@njit(cache=True)
def vectorized_portfolio_metrics(
