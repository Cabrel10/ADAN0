        ...
        for key, value in list(config_copy.items()):
            if isinstance(value, str):
                ...
                def replace_env_var(match):
                    ...
                    # SOTA: Never crash on missing env vars -- silently default
                    # to empty string so the bot works on pristine machines.
                    return ""
        ...
        for key, value in list(config_copy.items()):
            if isinstance(value, str) and '${' in value:
                    ...
                    try:
                        ...
                        for part in parts:
                            ...
                            if resolved == part and part not in os.environ:
                                # SOTA: Never crash on unresolved vars -- return
                                # empty string so the bot works on pristine machines.
                                import logging as _lg
                                _lg.getLogger("config_loader").debug(
                                    f"Unresolved var '{var_path}' -- defaulting to empty"
                                )
                                return ""
                ...
                try:
                    ...
                    # Graceful fallback for unresolved variables
                    config_copy[key] = ""
