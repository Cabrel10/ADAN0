...
document.addEventListener("DOMContentLoaded", async function () {
    ...
    // Get the blog post path from the `data-path` attribute
    const blogPostPath = blogPostsContainer.getAttribute("data-path");
    const currentDomain = window.location.hostname;

    if (!blogPostPath) {
        console.error("Error: No data-path attribute found on #blogPosts.");
        blogPostsContainer.innerHTML = `<p class="text-red-500">Error: No blog post path specified.</p>`;
        return;
    }

    try {
        // Fetch the blog post content
        const response = await fetch(blogPostPath);
        if (!response.ok) {
            throw new Error(`Failed to load blog post from ${blogPostPath}`);
        }
        const blogPostContent = await response.text();

        // Inject blog post content into the container
        blogPostsContainer.innerHTML = blogPostContent;

        // Update links dynamically
        const internalLinks = blogPostsContainer.querySelectorAll('a[data-dynamic-link="true"]');
        internalLinks.forEach(link => {
            const href = link.getAttribute("href");
            if (href.startsWith("/")) {
                link.href = `https://${currentDomain}${href}`;
            }
        });
    } catch (error) {
        console.error("Error loading blog post:", error);
        blogPostsContainer.innerHTML = `<p class="text-red-500">Error loading blog post. Please try again later.</p>`;
    }
});
