...
_STEP_RE = re.compile(r"(?:\[STEP\s+(\d+)\]|\[STEP\]\s+Starting step\s+(\d+))")
...
def parse_progress() -> dict[str, Any]:
    ...
    for line in tail.splitlines():
        ...
        if m:
            last_ts = max(last_ts or 0, int(m.group(1)))
        # Heavy Ray/PBT workers emit an authoritative environment step on every
        # decision instead of SB3's table-formatted total_timesteps. Taking the
        # maximum across the interleaved worker log keeps Mission Control live
        # without fabricating or averaging progress.
        step_match = _STEP_RE.search(line)
        if step_match:
            worker_step = int(step_match.group(1) or step_match.group(2))
            last_ts = max(last_ts or 0, worker_step)
