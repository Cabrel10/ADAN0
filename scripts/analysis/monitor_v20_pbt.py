...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=15.0,
        help=(
            "Delay population-count alerts while trials initialize. This must cover "
            "the longest expected first rollout when fail-stop is enabled."
        ),
    )
...
def main() -> int:
    ...
    while True:
        ...
        if arena.get("recent_records", 0) >= 1000 and max_duration_rate is not None and max_duration_rate > 0.60:
            # MaxDuration is a policy-performance symptom, not proof of state or
            # accounting corruption. Keep it visible but never feed fail-stop.
            alerts.append(
                f"WARN arena: MaxDuration={max_duration_rate:.1%} over recent closings"
            )
        if (
            time.time() - monitor_started > args.startup_grace_minutes * 60
