#!/usr/bin/env python3
"""
ADAN Colab Pipeline Notebook Generator
========================================
Generates ``ADAN_Colab_Pipeline.ipynb`` with cells for:
  1. Setup: Clone repo, install dependencies
  2. Data: Download 50,000 CCXT candles
  3. Train: Run PBT with 4 profiles on T4 GPU
  4. Extract: Best model to production/
  5. Live: Paper trading instructions

Usage:
    python scripts/generate_colab_notebook.py
    python scripts/generate_colab_notebook.py --output notebooks/ADAN_Colab_Pipeline.ipynb
"""
...
def create_notebook_cell(cell_type: str, source: list, metadata: dict = None):
...
def generate_notebook():
    """Generate the complete ADAN Colab Pipeline notebook."""
    cells = []
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate ADAN Colab Pipeline Notebook"
    )
    parser.add_argument(
        "--output", default="ADAN_Colab_Pipeline.ipynb",
        help="Output notebook path (default: ADAN_Colab_Pipeline.ipynb)",
    )
    ...
    print(f"[ADAN] Colab notebook generated: {output_path}")
    print(f"  Cells: {len(notebook['cells'])}")
    print(f"  Upload to Google Colab and run all cells.")
    print(f"\n  Direct link (after pushing to GitHub):")
    print(f"  https://colab.research.google.com/github/Cabrel10/ADAN0/blob/main/{args.output}")
