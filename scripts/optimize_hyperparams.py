...
from src.adan_trading_bot.data_processing.data_loader import (
    ChunkedDataLoader,
...
def objective(trial: optuna.Trial) -> float:
    ...
    try:
        ...
        reward_params = {
            ...
            "missed_opportunity_penalty": trial.suggest_float(
                "missed_opportunity_penalty", -0.65, -0.065
            ),
            ...
            "frequency_min_positions_5m": trial.suggest_int(
                "frequency_min_positions_5m", 1, 10
            ),
            "frequency_max_positions_5m": trial.suggest_int(
                "frequency_max_positions_5m", 10, 30
            ),
            "frequency_min_positions_1h": trial.suggest_int(
                "frequency_min_positions_1h", 1, 5
            ),
            "frequency_max_positions_1h": trial.suggest_int(
                "frequency_max_positions_1h", 5, 15
            ),
            "frequency_min_positions_4h": trial.suggest_int(
                "frequency_min_positions_4h", 0, 3
            ),
            "frequency_max_positions_4h": trial.suggest_int(
                "frequency_max_positions_4h", 1, 5
            ),
            "frequency_total_daily_min": trial.suggest_int(
                "frequency_total_daily_min", 1, 10
            ),
            "frequency_total_daily_max": trial.suggest_int(
                "frequency_total_daily_max", 10, 25
            ),
            "frequency_bonus_weight": trial.suggest_float(
                "frequency_bonus_weight", 0.1, 1.0
            ),
            "frequency_penalty_weight": trial.suggest_float(
                "frequency_penalty_weight", 0.01, 0.5
            ),
            "frequency_grace_period_steps": trial.suggest_int(
                "frequency_grace_period_steps", 50, 200
            ),
            "duration_tracking_5m_max_duration_steps": trial.suggest_int(
                "duration_tracking_5m_max_duration_steps", 24, 96
            ),
            "duration_tracking_5m_optimal_duration": trial.suggest_int(
                "duration_tracking_5m_optimal_duration", 12, 48
            ),
            "duration_tracking_1h_max_duration_steps": trial.suggest_int(
                "duration_tracking_1h_max_duration_steps", 12, 48
            ),
            "duration_tracking_4h_max_duration_steps": trial.suggest_int(
                "duration_tracking_4h_max_duration_steps", 6, 24
            ),
            "force_trade_steps": trial.suggest_int("force_trade_steps", 50, 200),
        }

        dbe_params = {
            "dbe_aggressiveness_decay": trial.suggest_float(
                "dbe_aggressiveness_decay", 0.9, 0.999
            ),
            "dbe_volatility_guard": trial.suggest_float(
                "dbe_volatility_guard", 0.1, 0.5
            ),
            "dbe_volatility_threshold": trial.suggest_float(
                "dbe_volatility_threshold", 0.01, 0.05
            ),
            "dbe_trend_threshold": trial.suggest_float(
                "dbe_trend_threshold", 0.01, 0.1
            ),
        }

        trading_rules_params = {
            "trading_rules_stop_loss_pct": trial.suggest_float(
                "trading_rules_stop_loss_pct", 0.01, 0.05
            ),
            "trading_rules_take_profit_pct": trial.suggest_float(
                "trading_rules_take_profit_pct", 0.02, 0.1
            ),
            "trading_rules_max_position_steps": trial.suggest_int(
                "trading_rules_max_position_steps", 100, 1000
            ),
        }
        ...
        position_sizing_params = {
            "initial_position_size": trial.suggest_float(
                "initial_position_size", 0.9, 0.95
            )
        }
        ...
        for param, value in all_hyperparams.items():
            ...
            if param in [
                ...
                param.startswith("frequency_")
                or param.startswith("duration_")
                or param == "force_trade_steps"
            ):
        ...
        for i in range(n_envs):
            env_log_dir = os.path.join(
                project_root, "logs", f"optuna_trial_{trial.number}_env_{i}"
            )
        ...
        policy_kwargs_for_ppo = copy.deepcopy(
            temp_config["agent"]["features_extractor_kwargs"]["policy_kwargs"]
        )
        activation_fn_map = {
            "ReLU": nn.ReLU,
            "Tanh": nn.Tanh,
        ...
        # Retrieve and log final metrics from SubprocVecEnv
        try:
            portfolio_managers = env.get_attr("portfolio_manager")
            if portfolio_managers and len(portfolio_managers) > 0:
                metrics_summary = portfolio_managers[0].metrics.get_metrics_summary()
                ...
                closed_trades = metrics_summary.get("executed_trades_closed", 0)
                win_rate = metrics_summary.get("win_rate", 0.0)
                profit_factor = metrics_summary.get("profit_factor", 0.0)
                total_pnl = metrics_summary.get("total_pnl", 0.0)

                logger.info(
                    f"Trial {trial.number} finished - Trades: {executed_trades} opened, {closed_trades} closed"
                )
                logger.info(
                    f"Trial {trial.number} - Win Rate: {win_rate:.2%}, Profit Factor: {profit_factor:.2f}, Total PnL: ${total_pnl:.2f}"
                )
                logger.info(
                    f"Trial {trial.number} - Valid: {valid_attempts}, Invalid: {invalid_attempts}, Sharpe: {final_sharpe_ratio:.4f}"
                )

                # Save comprehensive metrics as user attributes
                ...
                trial.set_user_attr("executed_trades_closed", closed_trades)
                trial.set_user_attr("win_rate", win_rate)
                trial.set_user_attr("profit_factor", profit_factor)
                trial.set_user_attr("total_pnl", total_pnl)
            else:
                logger.warning(f"Trial {trial.number}: No portfolio manager found")
                final_sharpe_ratio = -1.0
        except Exception as e:
            logger.error(f"Trial {trial.number}: Error retrieving metrics: {e}")
            final_sharpe_ratio = -1.0
...
if __name__ == "__main__":
    ...
    # Create an Optuna study to maximize Sharpe Ratio with TPE Sampler and Pruner
